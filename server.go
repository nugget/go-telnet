...
func (server *Server) Serve(listener net.Listener) error {
	...
	for {
		...
		server.Conn, err = listener.Accept()
		if err != nil {
			//@TODO: Could try to recover from certain kinds of errors. Maybe waiting a while before trying again.
			return err
		}
		logger.Debugf("Received new connection from %q.", server.Conn.RemoteAddr())

		// Handle the new TELNET client connection by spawning
		// a new goroutine.
		go server.handle(server.Conn, handler)
		logger.Debugf("Spawned handler to handle connection from %q.", server.Conn.RemoteAddr())
	}
}
...
func (server *Server) handle(c net.Conn, handler Handler) {
	...
	ctx = ctx.InjectLogger(logger)
	ctx = ctx.InjectConn(c)
